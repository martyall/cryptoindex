...
- Every paragraph must belong to at least one unit, except a `footnote` or `caption`: include one only where it belongs to the point a unit makes (a caption with the figure its unit discusses, a footnote whose content the argument uses), and leave out the rest, such as a footnote that only points at a file or a source. Units are listed in reading order.
- Units may overlap by a paragraph when one paragraph genuinely belongs to two points. Do not overlap otherwise.
- A lone heading-like fragment joins the unit it belongs to.
- The last unit of the section may be cut off mid-argument; describe what is there.

## Anchors

If a unit is built around a labelled formal block (Theorem 3.1, Lemma 2, Definition 4.2, Corollary, Proposition, Algorithm 1, Game G_0, Construction 5, Example 2.3, Exercise 7, …), set:

- `anchor_label`: the label, copied exactly as it appears in that paragraph's text, character for character, including its LaTeX if any. It is checked against the text, and a label not found there rejects your whole reply. Include only the kind and number (`Theorem 3.1`), not a title in parentheses or the trailing period.
- `anchor_pos`: the position of the paragraph where the label appears.
- `anchor_term`: the block's own word, lowercased, as the text writes it: `theorem`, `lemma`, `corollary`, `proposition`, `claim`, `definition`, `construction`, `algorithm`, `protocol`, `game`, `experiment`, `hybrid`, `example`, `exercise`, `remark`, `figure`, `table`, or whatever else this document uses. It must be a word of `anchor_label`. Do not translate it into another document's vocabulary.
- `anchor_kind`: which of these groups the block belongs to:
  - `theorem`: a theorem, lemma, proposition, corollary, or claim;
  - `definition`;
  - `algorithm`: an algorithm, procedure, protocol, or construction, in pseudocode or prose;
...
If the block is unnumbered or has no label, or the unit has no formal block, set all four to null.
