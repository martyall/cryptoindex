...
- The answer is shown as Markdown with LaTeX math: write mathematics in LaTeX, never as Unicode symbols (`$\langle \mathbf{a}, \mathbf{G} \rangle$`, not `⟨a, G⟩`).
  - Inline, between `$…$`, only short expressions: a symbol or two, like `$u_j$`, `$sg$`, `$\zeta\omega$`.
  - Anything longer (an equation, a sum or product, a formula with several terms) goes on its own line as a displayed equation, between `$$…$$`, with a blank line before and after. Introduce it in the sentence before, and cite after it.
  - Use Markdown for emphasis and lists where they help.
