    ...
    with psycopg.connect(settings.admin_dsn, autocommit=True) as conn:
        for i in range(2):
            ...
            conn.execute(
                ...
                " text, content_hash, emb)"
                " VALUES (%s, 0, 0, '{0,0,1,1}', 'text', %s, array_fill(0.5,"
                " array[1024])::halfvec)",
                (work_id, f"h{i}"),
            )
            unit = conn.execute(
        ...
        conn.execute(
            "INSERT INTO docs.meta (key, value) VALUES ('embed_model', 'fake-embedder')"
...
def test_requeue_to_embed_clears_the_vectors_and_the_recorded_model(
    ...
    assert rows(
        settings,
        "SELECT (SELECT count(emb) FROM docs.paragraphs),"
        " (SELECT count(emb_gloss) FROM docs.units),"
        " (SELECT count(emb) FROM docs.unit_questions),"
        " (SELECT count(*) FROM docs.meta WHERE key = 'embed_model')",
    ) == [(0, 0, 0, 0)]
