...
7. **Answer.** An answer handler (D28) calls retrieval tools, reads originals, and answers in prose whose main points cite paragraphs by location (D29, D30). The citation checker removes any citation of a paragraph the tools did not return in the session; the answer stays. Saying the documents do not answer the question is a valid answer.
