...
- **A time limit on each `claude_code` call** (for Phase 7's retry policy). Glossing the corpus (D18, `docs/CORPUS.md`) on 2026-09-22 ran into what looked like subscription throttling. One `claude -p` call (the mechanism before D31) sat idle for over 15 minutes, and the thesis took about seven hours instead of about twenty minutes. Nothing was lost, because each chunk's reply is stored as it arrives. But a stalled call blocks its stage with no limit and nothing in the log. The local OpenAI-format backend has a 900 s timeout and the Anthropic backend has the SDK's default; the `claude_code` backend has none.
