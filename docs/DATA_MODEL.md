...
- `anchor_label`, `anchor_pos`, `anchor_kind`, `anchor_term` (all null or all set; migrations 006 and 007) — the formal block that anchors the unit, if any, and the paragraph where its label appears verbatim. The same label is written to that paragraph's `block_label`.
